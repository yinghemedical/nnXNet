...
from nnxnet.preprocessing.resampling.utils import recursive_find_resampling_fn_by_name
import nnxnet
from batchgenerators.utilities.file_and_folder_operations import load_json, join

from nnxnet.imageio.reader_writer_registry import recursive_find_reader_writer_by_name
from nnxnet.utilities.find_class_by_name import recursive_find_python_class
from nnxnet.utilities.label_handling.label_handling import get_labelmanager_class_from_plans
...
if TYPE_CHECKING:
    from nnxnet.utilities.label_handling.label_handling import LabelManager
    from nnxnet.imageio.base_reader_writer import BaseReaderWriter
    from nnxnet.preprocessing.preprocessors.default_preprocessor import DefaultPreprocessor
    from nnxnet.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
...
class ConfigurationManager(object):
    def __init__(self, configuration_dict: dict):
        ...
        if 'architecture' not in self.configuration.keys():
            warnings.warn("Detected old nnX-Net plans format. Attempting to reconstruct network architecture "
                          "parameters. If this fails, rerun nnXNet_plan_experiment for your dataset. If you use a "
                          "custom architecture, please downgrade nnX-Net to the version you implemented this "
    ...
    def preprocessor_class(self) -> Type[DefaultPreprocessor]:
        preprocessor_class = recursive_find_python_class(join(nnxnet.__path__[0], "preprocessing"),
                                                         self.preprocessor_name,
                                                         current_module="nnxnet.preprocessing")
    ...
    @property
    def seg_index(self) -> List[List[int]]:
        return self.configuration['seg_index']
    
    @property
    def seg_index_1(self) -> List[List[int]]:
        return self.configuration['seg_index_1']
    
    @property
    def seg_index_2(self) -> List[List[int]]:
        return self.configuration['seg_index_2']
    
    @property
    def seg_ce_class_weights(self) -> List[List[int]]:
        return self.configuration['seg_ce_class_weights']
    
    @property
    def seg_ce_class_weights_1(self) -> List[List[int]]:
        return self.configuration['seg_ce_class_weights_1']

    @property
    def seg_ce_class_weights_2(self) -> List[List[int]]:
        return self.configuration['seg_ce_class_weights_2']
    
    @property
    def cls_task_index(self) -> List[List[int]]:
        return self.configuration['cls_task_index']
...
class PlansManager(object):
    ...
    def experiment_planner_class(self) -> Type[ExperimentPlanner]:
        ...
        experiment_planner = recursive_find_python_class(join(nnxnet.__path__[0], "experiment_planning"),
                                                         planner_name,
                                                         current_module="nnxnet.experiment_planning")
...
if __name__ == '__main__':
    from nnxnet.paths import nnXNet_preprocessed
    from nnxnet.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name

    plans = load_json(join(nnXNet_preprocessed, maybe_convert_to_dataset_name(3), 'nnXNetPlans.json'))
