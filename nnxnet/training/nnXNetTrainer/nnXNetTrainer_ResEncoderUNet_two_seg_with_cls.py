...
class nnXNetTrainer_ResEncoderUNet_two_seg_with_cls(nnXNetTrainer):
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        network = ResEncoderUNet_two_seg_with_cls(
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output_1, output_2, cls_pred_list = self.network(data)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # Prepare CSV header
        header = ['Epoch', 'Case_name', 'Case_cls_task_0_loss', 'Case_cls_task_1_loss']
       ...
       # Collect data for each case - process sequentially in order
        case_probs_task1 = []  # Aneurysm Present (task1)
        case_probs_task2 = []  # Other 13 categories (task2)
        case_targets_task1 = []
        case_targets_task2 = []
        ...
        # Process sequentially since val_outputs order matches keys order
        for i, output in enumerate(val_outputs):
            
            for b in range(batch_size):
                case_cls_task_0_loss = output['cls_task_0_loss']
                case_cls_task_0_losses.append(case_cls_task_0_loss)

                case_cls_task_1_loss = output['cls_task_1_loss']
                case_cls_task_1_losses.append(case_cls_task_1_loss)
            
            # task1: Aneurysm Present
            if 'cls_task_0_probs' in output:
                probs_task1 = output['cls_task_0_probs']
                targets_task1 = output['cls_task_0_targets']
            
            # task2: Other 13 categories
            if 'cls_task_1_probs' in output:
        ...
        for i, key in enumerate(keys):
            # Add probabilities and ground truth for each label
            ...
            for j, case_name in enumerate(key):
                ...
                # Create a row of data for each case
                row = [
