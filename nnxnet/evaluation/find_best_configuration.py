...
from nnxnet.ensembling.ensemble import ensemble_crossvalidations
from nnxnet.evaluation.accumulate_cv_results import accumulate_cv_results
from nnxnet.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
from nnxnet.paths import nnXNet_preprocessed, nnXNet_raw, nnXNet_results
from nnxnet.postprocessing.remove_connected_components import determine_postprocessing
from nnxnet.utilities.file_path_utilities import maybe_convert_to_dataset_name, get_output_folder, \
...
default_trained_models = tuple([
    {'plans': 'nnXNetPlans', 'configuration': '2d', 'trainer': 'nnXNetTrainer'},
    {'plans': 'nnXNetPlans', 'configuration': '3d_fullres', 'trainer': 'nnXNetTrainer'},
    {'plans': 'nnXNetPlans', 'configuration': '3d_lowres', 'trainer': 'nnXNetTrainer'},
    {'plans': 'nnXNetPlans', 'configuration': '3d_cascade_fullres', 'trainer': 'nnXNetTrainer'},
])
...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int]):
    ...
    for trained_model in model_dict:
        plans_manager = PlansManager(join(nnXNet_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id),
        ...
        if trained_model['configuration'] not in plans_manager.available_configurations:
            print(f"Configuration {trained_model['configuration']} not found in plans {trained_model['plans']}.\n"
                  f"Inferred plans file: {join(nnXNet_preprocessed, maybe_convert_to_dataset_name(dataset_name_or_id), trained_model['plans'] + '.json')}.")
            continue
...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
                               plans_identifier: str = 'nnXNetPlans', trainer_name: str = 'nnXNetTrainer',
    ...
    predict_command = f'nnXNet_predict -d {dataset_name_or_id} -i {input_folder} -o {output_folder} -f {fold_str} ' \
...
def find_best_configuration(dataset_name_or_id,
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                output_folder_ensemble = join(nnXNet_results, dataset_name, 'ensembles', identifier)
                ...
                compute_metrics_on_folder(join(nnXNet_preprocessed, dataset_name, 'gt_segmentations'),
    ...
    determine_postprocessing(all_results[best_key]['source'], join(nnXNet_preprocessed, dataset_name, 'gt_segmentations'),
    ...
    save_json(return_dict, join(nnXNet_results, dataset_name, 'inference_information.json'))  # save this so that we don't have to run this
    ...
    # print it
    print_inference_instructions(return_dict, instructions_file=join(nnXNet_results, dataset_name, 'inference_instructions.txt'))
    return return_dict
...
def print_inference_instructions(inference_info_dict: dict, instructions_file: str = None):
    ...
    if is_ensemble:
        ...
        _print_and_maybe_write_to_file(f"nnXNet_ensemble -i {output_folder_str} -o {output_ensemble} -np {default_num_processes}")

    _print_and_maybe_write_to_file("\n***Once inference is completed, run postprocessing like this:***\n")
    _print_and_maybe_write_to_file(f"nnXNet_apply_postprocessing -i OUTPUT_FOLDER -o OUTPUT_FOLDER_PP "
...
def find_best_configuration_entry_point():
    ...
    parser.add_argument('-p', nargs='+', required=False, default=['nnXNetPlans'],
                        help='List of plan identifiers. Default: nnXNetPlans')
    ...
    parser.add_argument('-tr', nargs='+', required=False, default=['nnXNetTrainer'],
                        help='List of trainers. Default: nnXNetTrainer')
...
def accumulate_crossval_results_entry_point():
    ...
    parser.add_argument('-p', type=str, required=False, default='nnXNetPlans',
                        help='Plan identifier in which to search for the specified configuration. Default: nnXNetPlans')
    parser.add_argument('-tr', type=str, required=False, default='nnXNetTrainer',
                        help='Trainer class. Default: nnXNetTrainer')
    args = parser.parse_args()
