...
class nnXNetPredictor(object):
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        predicted_logits_1, predicted_logits_2, cls_predicted_logits_list = self.predict_logits_from_preprocessed_data(dct['data'])
        predicted_logits_1 = predicted_logits_1.cpu()
        predicted_logits_2 = predicted_logits_2.cpu()
        ...
        if output_file_truncated is not None:
            export_prediction_from_logits(predicted_logits_1, dct['data_properties'], self.configuration_manager,
                                          self.plans_manager, self.dataset_json, output_file_truncated,
                                          save_or_return_probabilities)
            export_prediction_from_logits(predicted_logits_2, dct['data_properties'], self.configuration_manager,
                                          self.plans_manager, self.dataset_json, output_file_truncated,
                                          save_or_return_probabilities)
        
        else:
            ret_1 = convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits_1, self.plans_manager,
                                                                              self.configuration_manager,
                                                                              self.label_manager,
                                                                              dct['data_properties'],
            ...
            ret_2 = convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits_2, self.plans_manager,
                                                                              self.configuration_manager,
                                                                              self.label_manager,
                                                                              dct['data_properties'],
                                                                              return_probabilities=
            ...
            return ret_1, ret_2, cls_prob_list
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            if prediction_1 is None and prediction_2 is None and cls_pred_list is None:
                prediction_1, prediction_2, cls_pred_list = self.predict_sliding_window_return_logits(data)
                prediction_1 = prediction_1.to('cpu')
                prediction_2 = prediction_2.to('cpu')
                for t_i in range(self.num_cls_task):
                    cls_pred_list[t_i] = cls_pred_list[t_i].to('cpu')
            else:
                prediction_sw_1, prediction_sw_2, cls_pred_list_tmp = self.predict_sliding_window_return_logits(data)
                prediction_sw_1 = prediction_sw_1.to('cpu')
                prediction_sw_2 = prediction_sw_2.to('cpu')
                prediction_1 += prediction_sw_1
                prediction_2 += prediction_sw_2

                for t_i in range(self.num_cls_task):
                    cls_pred_list[t_i] += cls_pred_list_tmp[t_i].to('cpu')

        if len(self.list_of_parameters) > 1:
            prediction_1 /= len(self.list_of_parameters)
            prediction_2 /= len(self.list_of_parameters)
        ...
        return prediction_1, prediction_2, cls_pred_list
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prediction_1, prediction_2, cls_pred_list = self.network(x)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction_1_flip, prediction_2_flip, flipped_cls_pred_list = self.network(torch.flip(x, axes))
                for t_i in range(self.num_cls_task):
                    cls_pred_list[t_i] += flipped_cls_pred_list[t_i]
                prediction_1 += torch.flip(prediction_1_flip, axes)
                prediction_2 += torch.flip(prediction_2_flip, axes)
            prediction_1 /= (len(axes_combinations) + 1)
            prediction_2 /= (len(axes_combinations) + 1)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        predicted_logits_1 = predicted_logits_2 = n_predictions = prediction_1 = prediction_2 = gaussian = workon = None
        ...
        try:
            ...
            predicted_logits_1 = torch.zeros((len(self.seg_index_1) + 1, *data.shape[1:]),
                                           dtype=torch.half,
                                           device=results_device)
            predicted_logits_2 = torch.zeros((len(self.seg_index_2) + 1, *data.shape[1:]),
                                            dtype=torch.half,
                                           device=results_device)
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction_1_batch, prediction_2_batch, cls_pred_list = self._internal_maybe_mirror_and_predict(workon)
                prediction_1 = prediction_1_batch[0].to(results_device)
                prediction_2 = prediction_2_batch[0].to(results_device)

                if self.use_gaussian:
                    prediction_1 *= gaussian
                    prediction_2 *= gaussian
                
                n_predictions[sl[1:]] += gaussian

                seg = torch.argmax(prediction_1, dim=0)
                ...
                predicted_logits_1[sl] += prediction_1
                predicted_logits_2[sl] += prediction_2
                n_predictions[sl[1:]] += gaussian

            predicted_logits_1 /= n_predictions
            predicted_logits_2 /= n_predictions
            # check for infs
            if torch.any(torch.isinf(predicted_logits_1)):
                raise RuntimeError('Encountered inf in predicted array. Aborting... If this problem persists, '
                                   'reduce value_scaling_factor in compute_gaussian or increase the dtype of '
                                   'predicted_logits to fp32')
            if torch.any(torch.isinf(predicted_logits_2)):
                raise RuntimeError('Encountered inf in predicted array. Aborting... If this problem persists, '
                                   'reduce value_scaling_factor in compute_gaussian or increase the dtype of '
                                   'predicted_logits to fp32')
        except Exception as e:
            del predicted_logits_1, predicted_logits_2, n_predictions, prediction_1, prediction_2, gaussian, workon
            empty_cache(self.device)
            empty_cache(results_device)
            raise e
        return predicted_logits_1, predicted_logits_2, cls_predicted_logits_list
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                ...
                try:
                    predicted_logits_1, predicted_logits_2, cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                           self.perform_everything_on_device)
                except RuntimeError:
                    print(
                        'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                    empty_cache(self.device)
                    predicted_logits_1, predicted_logits_2, cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers, False)
            else:
                predicted_logits_1, predicted_logits_2, cls_predicted_logits_list = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                       self.perform_everything_on_device)
            ...
            empty_cache(self.device)
            # revert padding
            predicted_logits_1 = predicted_logits_1[(slice(None), *slicer_revert_padding[1:])]
            predicted_logits_2 = predicted_logits_2[(slice(None), *slicer_revert_padding[1:])]
        return predicted_logits_1, predicted_logits_2, cls_predicted_logits_list
